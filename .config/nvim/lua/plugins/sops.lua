...
return {
	...
	config = function()
		...
		-- Функция для поиска .sops.yaml в родительских директориях
		local function find_sops_config(start_path)
			local current = start_path
			while current ~= "/" do
				local sops_file = current .. "/.sops.yaml"
		...
		vim.api.nvim_create_autocmd("BufWriteCmd", {
			...
			callback = function()
				if vim.b.sops_enc then
					...
					-- Ищем .sops.yaml в родительских директориях
					local sops_root = find_sops_config(file_dir)

					if not sops_root then
						print("❌ .sops.yaml not found in parent directories")
						return
					end
					...
					-- Зашифровываем ИЗ КОРНЯ РЕПОЗИТОРИЯ (где есть .sops.yaml)
					local cmd = string.format(
						"cd %s && sops -e %s > %s",
						vim.fn.shellescape(sops_root),
					...
					if vim.v.shell_error == 0 then
						...
						print("❌ Failed to encrypt SOPS file: " .. result)
					end
				else
